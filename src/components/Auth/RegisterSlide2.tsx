...
interface RegisterSlide2Props {
  updateFormData: (data: { name?: string; age?: number | null }) => void; // More specific type
  prevSlide: () => void;
...
const RegisterSlide2: React.FC<RegisterSlide2Props> = ({ updateFormData, prevSlide, nextSlide, currentSlide, totalSlides }) => {
  ...
  const handleNext = () => {
    if (validateForm()) {
      ...
      updateFormData({ name, age: age >= 16 ? age : null }); // Pass age only if valid
    ...
    <div>
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        <Button
           ...
           onClick={handleNext} // Use handleNext now
        >
          Weiter
        </Button>
      </div>
